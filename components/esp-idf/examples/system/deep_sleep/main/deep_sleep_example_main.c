...
#include "esp_system.h"     // <- for esp_reset_reason, esp_chip_info
#include "esp_chip_info.h"  // <- for esp_chip_info, esp_system.h no longer re-exports esp_chip_info().
...
{
    // --- New: show reset reason + cold-boot vs deep-sleep wake ---
    ...
    if (reason == ESP_RST_DEEPSLEEP) {
        ...
        ESP_LOGI(TAG, "Cold boot / other reset.");
    }

    // Optional: some basic chip info (harmless on all boots)
    esp_chip_info_t chip_info;
    esp_chip_info(&chip_info);
    ESP_LOGI(TAG, "ESP32-S3 cores: %d, WiFi%s%s",
             chip_info.cores,
...
{
    const int wakeup_time_sec = 20;   // change to 10 if you want faster cycles
    printf("Enabling timer wakeup, %ds\n", wakeup_time_sec);
    ESP_ERROR_CHECK(esp_sleep_enable_timer_wakeup(wakeup_time_sec * 1000000LL));
}
